...
const QUICK_PROMPTS = [
  ...
  'What changed in my mission?',
  'Find a safer alternative',
  'Zone B kab suitable hoga?',
];

export default function AskOrcaDrawer({
  userOrigin,
  zones,
  trackedDecisions,
  onDecisionReceived,
  ...
  const handleSend = async (queryText?: string) => {
    ...
    try {
      ...
      const errorMsg: ChatMessage = {
        id: `err-${Date.now()}`,
        sender: 'orca',
        text: 'Natural-language assistant temporarily unavailable. You can still select and evaluate zones using the GIS map.',
        timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
      };
  ...
  return (
    <div className="bg-slate-900/90 border border-slate-800 rounded-2xl p-4 md:p-5 flex flex-col justify-between shadow-2xl backdrop-blur h-full">
      ...
      <div className="pb-2.5 border-b border-slate-800 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-bold text-white text-xs uppercase tracking-wider font-mono">
            Ask ORCA (Multilingual AI)
          </h3>
        </div>
        <span className="text-[10px] font-mono text-slate-400">
          Gemini 2.5 Flash • English / Hindi / Hinglish
        </span>
      </div>
      ...
      <div className="flex-1 overflow-y-auto py-2.5 space-y-2.5 pr-1 text-xs">
            ...
            <div
              className={`max-w-[88%] p-3 rounded-2xl ${
                m.sender === 'user'
                  ...
                  : 'bg-slate-950/95 text-slate-200 border border-slate-800 rounded-bl-none shadow-md'
              }`}
            >
        ...
        {loading && (
          <div className="flex items-center gap-2 p-2.5 bg-slate-950/80 border border-slate-800 rounded-2xl rounded-bl-none max-w-[85%] text-xs text-slate-400 font-mono">
      ...
      <div className="pt-2 border-t border-slate-800">
        <div className="flex items-center gap-1.5 overflow-x-auto pb-2 scrollbar-none">
          {QUICK_PROMPTS.map((p, idx) => (
            <button
              ...
              className="text-[10px] px-2.5 py-1 rounded-full bg-slate-950 border border-slate-800 hover:border-cyan-600 text-slate-300 whitespace-nowrap transition-all font-mono"
            >
              {p}
            </button>
          ))}
        ...
        >
          <input
            ...
            disabled={loading}
            className="flex-1 px-3 py-2 bg-slate-950 border border-slate-800 focus:border-cyan-500 rounded-xl text-xs text-white placeholder:text-slate-500 outline-none transition-all font-sans"
          />
          <button
            type="submit"
            disabled={!input.trim() || loading}
            className="px-3.5 py-2 bg-cyan-600 hover:bg-cyan-500 disabled:opacity-40 text-white font-bold rounded-xl text-xs transition-all shadow-md font-mono"
          >
            Send
          </button>
        </form>
